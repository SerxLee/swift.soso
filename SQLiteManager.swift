...
//  serx.lee@gmail.com
//
//  Created by Serx on 2016-01-25 11:34:43
//  Copyright © 2016 serx. All rights reserved.
//

/*
  	sqlite3.h 是以C语言为框架的，函数是都以sqlite3开始的
*/
...
class SQLiteManager {
    ...
    //数据库操作串行队列，使用多线程去执行
    private let queue = dispatch_queue_create("serxSqlite", DISPATCH_QUEUE_SERIAL)
    ...
    private func recordDict(stmt: COpaquePointer) -> [String : AnyObject] {
        ...
        for col in 0..<colCount {
            ...
            case SQLITE_BLOB:
                ...
                //获取bytes array[]
//                let lim: [UInt8]
//                lim = [UInt8](UnsafeBufferPointer(
//                    start: UnsafePointer(bytes), count: length
//                    ))
                value = NSData(bytes: [UInt8](UnsafeBufferPointer(
                    start: UnsafePointer(bytes), count: length
                    )), length: 2)
